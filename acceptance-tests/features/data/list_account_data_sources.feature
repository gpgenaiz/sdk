Feature: list account data sources
  To be able to list data sources available to an account
  As an authenticated user
  I should be able to login to a broker, create and publish a DataLink
  I should be able to initialize a locker and add a data source for the Datalink
  I should be able to publish the locker source and list it from the broker
  ...
  Scenario: login account data sources
  ...
  Scenario: add data source for data link
    ...
    And the following parameters
      ...
      | myLocker.bin | SIzlR0a$ | myLockerSrc | com.genaiz.test/datalink-1 | 1.0.0           |       |
  ...
  Scenario: create data source for account
    Given the scenario "update data source secret property" ran with condition "service_completed_successfully"
    And the following parameters
  ...
  Scenario: list data sources for account
    Given the scenario "create data source for account"
    And the following parameters
      | oem             | handle     | version | name         |
      | com.genaiz.test | datalink-1 | 1.0.0   | Locker src 1 |
    When I run the command "dt src list <oem>/<handle>:<version>"
    Then I should have a data source list with first item listed named "<name>" for datalink "<oem>/<handle>:<version>" with "1" property
